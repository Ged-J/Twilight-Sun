...
namespace Pathfinding.ECS {
	using Pathfinding;
	public partial struct StartOffMeshLinkTransitionJob {
		...
		public static System.Collections.Generic.IEnumerable<object> DefaultOnTraverseOffMeshLink (AgentOffMeshLinkTraversalContext ctx) {
			...
			var dirInPlane = ctx.movementPlane.ToWorld(ctx.movementPlane.ToPlane(linkInfo.secondPosition - linkInfo.firstPosition), 0);
			var rot = quaternion.LookRotationSafe(dirInPlane, up);

			while (!ctx.MoveTowards(linkInfo.firstPosition, rot, true, false).reached) yield return null;
			ctx.DisableLocalAvoidance();

			while (!ctx.MoveTowards(linkInfo.secondPosition, rot, true, false).reached) yield return null;
