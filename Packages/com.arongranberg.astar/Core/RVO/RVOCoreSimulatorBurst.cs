...
namespace Pathfinding.RVO {
	...
	public class SimulatorBurst {
		...
		JobHandle UpdateInternal<T>(JobHandle dependency, float deltaTime, bool drawGizmos, Allocator allocator) where T : struct, IMovementPlaneWrapper {
			...
			NavmeshEdges.NavmeshEdgeData navmeshEdgeData;
			bool hasAstar = AstarPath.active != null;
			if (hasAstar) {
				navmeshEdgeData = AstarPath.active.hierarchicalGraph.navmeshEdges.GetNavmeshEdgeData(out navmeshEdgeDataLock);
			} else {
				navmeshEdgeData = NavmeshEdges.NavmeshEdgeData.CreateEmpty(allocator);
